...
class UssSampler:
    def __init__(self, x, y, neighbor=10, plabel=1, is_noise_filter=False):
        self.x = np.array(x)
        self.y = np.array(y).ravel()
        self.plabel = plabel  # 少数类标签
        self.neighbor = neighbor
        self.is_noise_filter = is_noise_filter  # 是否过滤噪音数据
    ...
    def filter_noise(self):
        ...
        knn = NearestNeighbors(n_neighbors=60)  # 尽量过滤混合较多的邻居
        knn.fit(self.x)
        ...
        for i in range(lens):
            if self.y[i] == 1:
                ...
                # print(nn_matrix[i])
                # break
                noise_flag = self.is_noise(nn_matrix[i])
                if not noise_flag:
                    M.append(i)
                else:
                    # print(noise_flag)
    ...
    def is_noise(self, row_idx):
        noise_flag = True
        for i, idx in enumerate(row_idx):
            if i == 0:  # knn会把本身包括进来，需要跳过
                continue
            if self.y[idx] == 1:
                noise_flag = False
        return noise_flag
    ...
    def get_balance_data(self, ratio=1.2):
        ...
        # inrobust_idx = self.inrobust_idx()  # 对于算法不鲁棒的数据
        # balan_major_idx.extend(inrobust_idx)
        # balan_major_idx = list(set(balan_major_idx))
...
if __name__ == '__main__':
    ...
    data = pd.read_csv('train_.csv').fillna(0)
    x = data[x_col]
    y = data[y_col]

    uss = UssSampler(x, y,is_noise_filter=True)
    x_, y_ = uss.get_balance_data()
    print(len(y_))
    print(uss.inrobust_idx())
